...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getattr__(self, name):
        if (name not in {f.name for f in self.transform.flattened_fields}) and (not name == "preds"):
    ...
    def build(
        ...
        if not self.cache:
            ...
            # TODO: check if all same length
            #for i in self.sentences:
            #    l = len(i.values[0])
            #    assert all(len(f) == l for f in i.values), "Fields do not have the same length"
        else:
            # TODO: check if we enter this
        ...
        # TODO: check what happends in the loader
