...
class Sentence(object):
    ...
    def numericalize(self, fields):
        ...
        prev = None
        for f in fields:
            ##print(len(getattr(self, f.name)))
            #if prev is not None:
            #    assert len(getattr(self, f.name)) == prev
            #prev = len(getattr(self, f.name))
            #print("f.name", f.name)
            #print("self.values", self.values)
            #print("self.maps[f.name]", self.maps[f.name])
            #print("getattr(self, f.name)", getattr(self, f.name))
            #print("next(f.transform([getattr(self, f.name)]))", next(f.transform([getattr(self, f.name)])))
            #print(getattr(self, f.name))
            #print(len(getattr(self, f.name)))
            self.fields[f.name] = next(f.transform([getattr(self, f.name)]))    # HERE
            #print(len(next(f.transform([getattr(self, f.name)]))))#
