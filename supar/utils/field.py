    ...
    def transform(self, sequences: Iterable[List[str]]) -> Iterable[torch.Tensor]:
        for seq in sequences:
            ...
            if self.bos:
                ...
                #print("after bos", len(seq))
            if self.delay > 0:
                seq = seq + [[self.pad_index] for _ in range(self.delay)]

                #print("after delay", len(seq))
            if self.eos:
                seq = seq + [[self.eos_index]]

                #print("after eos", len(seq))
            if self.fix_len > 0:
                seq = [ids[:self.fix_len] for ids in seq]

                #print("after fix_len", len(seq))

            padded = pad([torch.tensor(ids, dtype=torch.long) for ids in seq], self.pad_index)

            #print("after padded", len(padded))
            if len(z) +3 == len(padded):
                raise Exception(f"{z}, {padded}")
            yield padded
